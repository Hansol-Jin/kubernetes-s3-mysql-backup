...
BACKUPFILES="$CURRENT_DATABASE"_"backup"_"$current_date"
    ...
    if sqloutput=$(mysqldump -u $TARGET_DATABASE_USER -h $TARGET_DATABASE_HOST -p$TARGET_DATABASE_PASSWORD -P $TARGET_DATABASE_PORT $CURRENT_DATABASE 2>&1 > /tmp/$CURRENT_DATABASE_$current_date.sql)
    then
        ...
        then
            ...
            echo -e "Database backup failed to upload for $CURRENT_DATABASE at $(TZ='Asia/Seoul' date +'%Y-%m-%d %H:%M:%S'). Error: $awsoutput" | tee -a /tmp/kubernetes-s3-mysql-backup.log
            has_failed=true
        fi

    else
        echo -e "Database backup FAILED for $CURRENT_DATABASE at $(TZ='Asia/Seoul' date +'%Y-%m-%d %H:%M:%S'). Error: $sqloutput" | tee -a /tmp/kubernetes-s3-mysql-backup.log
        has_failed=true
    fi
